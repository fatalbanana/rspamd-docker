  ...
  steps: [
    {
      ...
      commands: [
        ...
        'python3 -mvenv venv',
        'source venv/bin/activate && pip3 install --no-cache --disable-pip-version-check --no-binary :all: robotframework tornado',
        'git clone -b ${DRONE_SEMVER_SHORT} https://github.com/rspamd/rspamd.git',
        'RSPAMD_INSTALLROOT=/usr source venv/bin/activate && robot --removekeywords wuks --exclude isbroken $DRONE_WORKSPACE/rspamd/test/functional/cases',
      ],
    },
  ],
